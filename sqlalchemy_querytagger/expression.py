try:
    ...
    from sqlalchemy.sql.elements import ClauseElement, _clone
    from sqlalchemy.sql.expression import text, _SelectBase
except ImportError:
    from sqlalchemy.sql.expression import Selectable, Executable, ClauseElement, _SelectBase, text, _generative, _clone

class TaggedClauseElement(Executable, ClauseElement):
...
class SelectableTaggedClauseElement(TaggedClauseElement, _SelectBase):
    def as_scalar(self):
...
def make_tagged_clause_element(statement, comment):
    if isinstance(statement, Selectable):
        return SelectableTaggedClauseElement(statement, comment)
    else:
        return TaggedClauseElement(statement, comment)
