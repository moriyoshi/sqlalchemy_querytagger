...
from .expression import make_tagged_clause_element
...
print base_exclusion
class TaggerOption(MapperOption):
...
def tag(comment):
    return TaggerOption(comment)

WrapperDescriptorType = type(object.__getattribute__)

class TaggerQuery(Query):
    _inner = None
    ...
    def _clone(self):
        return TaggerQuery(self._inner_clone(self._inner))

    def _compile_context(self, labels=True):
        context = self._inner_compile_context(self._inner, labels)
        comment = self._attributes.get(COMMENT_ATTR_KEY) 
        if comment is not None:
            context.statement = make_tagged_clause_element(context.statement, comment)
        return context
...
def add_location_tag(inner):
    caller = find_caller(build_exclusion(inner))
    if caller:
        comment = caller.strip().partition('\n')[0]
        inner = inner.options(TaggerOption(comment))
    return inner

def SessionFactoryFactory(class_, wrapper=lambda inner: add_location_tag(TaggerQuery(inner))):
    def retouch(kwargs):
